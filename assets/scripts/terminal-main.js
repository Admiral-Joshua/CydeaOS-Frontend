...
var CommandList = ["help", "echo ", "clear", "wget", "color", "connect ", "hide", "exit", "filebrowser", "rm", "del", "ren", "nmap", "pwordcrack", "ls", "BruteSSH ", "FTPBounce ", "MemesVirus", "hack"];
...
function handleTerminalInput(e) {
	...
	/*} else if (e.which == 8) {
		...
		let terminalLines = terminal1.value.split("\n");
		console.log(terminalLines);
		let Command = terminalLines[terminalLines.length - 1];
		//ProcessCommand(lastLine);
		let currentCommand = Command.replace("Terminal>", "", 1);
		let fullCommand = ""
		autoComplete.index += 1;
		/*if (autoComplete.alts.length > 0) {
			if (autoComplete.index > autoComplete.alts.length - 1) {
				autoComplete.index = 0;
			}
			let itemFound = false;
			for (let possibleCommand of autoComplete.alts) {
				if (possibleCommand.indexOf(currentCommand) != -1) {
					itemFound = true;
				}
			}
			if (itemFound == false) {
		...
		if (fullCommand == "") {
			/*for (let possibleCommand of CommandList) {
				if (possibleCommand.toLowerCase().indexOf(currentCommand.toLowerCase()) != -1) {
					autoComplete.alts.push(possibleCommand);
			...
			for (let possibleCommand of CommandList) {
				if (possibleCommand.toLowerCase().indexOf(currentCommand.toLowerCase()) != -1) {
					fullCommand = possibleCommand;
					console.log("Autocomplete Possibility Found - '" + possibleCommand + "'");
					break;
				}
			}
		}
	
		//terminalLines[terminalLines.length - 1] = "Terminal>" + fullCommand;
		//terminalLines.push("Terminal>" + fullCommand);
		console.log(terminalLines.pop());
		terminalLines.push(("Terminal>" + fullCommand));
		console.log(terminalLines);
		
		terminal1.value = terminalLines.join("\n");
